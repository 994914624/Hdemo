...
public class SARecommendAdapter   extends BaseMultiItemQuickAdapter<NFeedEntity.DataBean, BaseViewHolder> {
    /**
     ...
     */
    public SARecommendAdapter(List<NFeedEntity.DataBean> data) {
        super(data);
        addItemType(NFeedEntity.DataBean.CLICK_ITEM_VIEW, R.layout.item_news_1);
        addItemType(NFeedEntity.DataBean.CLICK_ITEM_VIEW_2, R.layout.item_detial_webview_ad);
        addItemType(NFeedEntity.DataBean.FEED_ITEM_1, R.layout.item_feed_1);
    }



    @Override
    protected void convert(BaseViewHolder helper, NFeedEntity.DataBean item) {
        switch (helper.getItemViewType()) {
            case NFeedEntity.DataBean.CLICK_ITEM_VIEW:
                try {
                    if(item!=null){
                        //TODO 设置数据
                        ((TextView)helper.getView(R.id.tv_item_1_news_title)).setText(String.format("%s",item.getName()+""));
                        ((TextView)helper.getView(R.id.tv_item_1_news_subtitle)).setText(String.format("¥%s",item.getPrice().substring(0,item.getPrice().indexOf("."))+""));
                ...
                break;
            case NFeedEntity.DataBean.CLICK_ITEM_VIEW_2:
                //TODO 设置数据
                ((TextView)helper.getView(R.id.tv_item_detial_webview_ad_text)).setText(String.format("%s",item.getName()+""));
                break;
            case NFeedEntity.DataBean.FEED_ITEM_1:
                try {
                    if(item!=null){
                        //TODO Feed
                        ((TextView)helper.getView(R.id.item_feed_1_tv_name)).setText(String.format("%s",item.getName()+""));
                        ((TextView)helper.getView(R.id.item_feed_1_tv_price)).setText(String.format("¥%s",item.getPrice().substring(0,item.getPrice().indexOf("."))+""));
                        TextView  tv_origin_price = ((TextView)helper.getView(R.id.item_feed_1_tv_origin_price));
                        if(!item.getPrice().equals(item.getOrigin_price())){
                            tv_origin_price.setText(String.format("价格 ¥%s",item.getOrigin_price().substring(0,item.getOrigin_price().indexOf("."))+""));
                        ...
                        tv_tags.setBackgroundColor(color);
